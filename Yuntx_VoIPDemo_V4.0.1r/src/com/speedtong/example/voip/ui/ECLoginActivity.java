 ...
 * @version 4.0
 */
public class ECLoginActivity extends ECSuperActivity implements
	...
	public void onClick(View v) {
		...
		case R.id.experience_login_submit:
			...
			hideSoftKeyboard();
			if (TextUtils.isEmpty(mAccountEdt.getText())
					|| TextUtils.isEmpty(mPasswordEdt.getText())) {
				ToastUtil.showMessage(R.string.tips_login_accountpwd_null);
				return;
			}
			mPostingdialog = new ECProgressDialog(this, R.string.login_posting);
			mPostingdialog.show();
			doLoginReuqest(mAccountEdt.getText().toString(), mPasswordEdt
					.getText().toString());
			break;
