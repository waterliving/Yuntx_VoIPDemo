 ...
 * @version 4.0
 */
public class ECLauncherUI extends FragmentActivity implements
	...
	public final BaseFragment getTabView(int tabIndex) {
		LogUtil.d(LogUtil.getLogUtilsTag(ECLauncherUI.class), "get tab index "
				+ tabIndex);
		if (tabIndex < 0) {
			return null;
		}
		...
		case TAB_NET_VOIP:
			mFragment = (TabFragment) Fragment.instantiate(this,
					NetVoipFragment.class.getName(), null);
			break;
		case TAB_NET_PHONE:
			mFragment = (TabFragment) Fragment.instantiate(this,
					NetPhoneFragment.class.getName(), null);
			break;
	...
	void initOverflowItems() {
		...
		try {
			...
			name = ContactSqlManager.getContact(clientUser.getUserId())
					.getNickname();
		} catch (Exception e) {
